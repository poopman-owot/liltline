...
function Begin(){
   ...
   let hit = false;
    ...
    // Function to handle button events (pressed/released)
    function handleButtonEvent(buttonText, isPressed) {
      console.log(buttonText + ' button ' + (isPressed ? 'pressed' : 'released'));

      // Update variables based on button presses/releases
      if (buttonText === 'Up') {
        moveUp = isPressed;
...
spacebarPressed = false
          }, 200); // Set 'hit' to false after 0.2 seconds
        }
      }

      // Add your custom logic here to do something when a button is pressed or released
      // You can use the 'moveUp', 'moveDown', and 'hit' variables in your custom logic.
    }
    ...
    function createButton(text) {
      ...
      // Add event listeners for touchstart and touchend to detect button press/release
      button.addEventListener('touchstart', function() {
        handleButtonEvent(text, true); // Button pressed
      });

      button.addEventListener('touchend', function() {
        handleButtonEvent(text, false); // Button released
      });

      // Set the button's flex grow property to 1 to make it stretch and fill available space
      button.style.flexGrow = '1';

      return button;
    }

    // Function to create buttons and apply styles in JavaScript
    function createButtons() {
      const buttonContainer = document.createElement('div');
      buttonContainer.style.position = 'fixed';
      buttonContainer.style.right = '10px';
      buttonContainer.style.top = '0';
      buttonContainer.style.height = '100%'; // Set the height to 100%
      ...
      const button1 = createButton('Up');
      const button2 = createButton('Hit');
      const button3 = createButton('Down');
      ...
      elm.main_view.appendChild(buttonContainer);
    }

    // Check if it's a mobile device before creating the buttons
...
requestAnimationFrame(animate);
    ...
    function streamAudio() {
      ...
      function analyzeBeats() {
        ...
        var threshold = audioAverageLevel / audioTicks;
        if (threshold > 130) {
          threshold = 130
        }
        ...
        //console.log(averageAmplitude > threshold + (averageAmplitude / 10),averageAmplitude, threshold + (averageAmplitude / 10))
        return {
          isLoudBeat: averageAmplitude > threshold + (averageAmplitude / 10),
